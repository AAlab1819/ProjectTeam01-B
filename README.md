...
Dynamic Programming is breaking down an optimization problem into simple sub-problems, and storing the solution to each sub-problem so that each sub-problem is solved only once. In order to solve this problem using DP, we use memoization which ensures that a method doesn't run for the same input more than once by keeping record of the results for the given input.

Memoization is used by using 'bool solve (int pos, int nopen)' and 'temp' to store the calculation. So the 'bool solve' function is to find out if a certain substring can be a valid sequence of brackets.
